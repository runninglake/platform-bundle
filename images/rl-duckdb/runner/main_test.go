...
func fakeDuckDB() int {
	...
	// THE RESULT'S COPY IS NOW GENERATED BY THE ENGINE, so the path this fake honours is
	// the one inside the statement that writes it — `TO ''<path>''` — rather than a COPY
	// spelled directly into the script. That is a real cost of the generated form worth
	// naming: this fake is a SECOND DECLARATION of the script's shape, and it broke, loudly
	// and in seven tests, the moment the shape changed. The nesting it matches is also the
	// thing that has to be right (see nestedSQLString): a single level of quoting parses as
	// a string, a stray identifier and a type, and this pattern would not match it either.
	if m := regexp.MustCompile(`TO ''([^']+)'' \(FORMAT PARQUET\);`).FindStringSubmatch(s); m != nil {
...
func TestSuccessfulQuery(t *testing.T) {
	...
	for _, frag := range []string{
		...
		// The result's COPY is generated by the engine from the result's own schema, so
